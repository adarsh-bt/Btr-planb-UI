...
const UserTourDiaryDetail = () => {
  ...
  const zoneId = authservice.getzone();
  if (!zoneId) {
    setSnackbar({ open: true, message: "No active zone found. Please select a zone first.", severity: "error" });
    return;
  }
...
const handleConfirmSubmit = async () => {
  ...
  try {
    ...
    if (isError) {
      ...
      setResultDialog({
        ...
        message: message || "Month submitted successfully"
      });
      await fetchUserTourEntries();
    }
  ...
  const handleEditEntry = (entry) => {
    if (entry.reportEntryType === "AUTO CAPTURED") {
      ...
      if (entry.schemesId) { setEditSelectedScheme(entry.schemesId); }
      else { setEditSelectedScheme(""); }
    }
              ...
              >
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, width: '250px', justifyContent: 'flex-end' }}>
                  <Button
                    ...
                    sx={{ backgroundColor: '#27ae60', '&:hover': { backgroundColor: '#229954' }, whiteSpace: 'nowrap', minWidth: '120px' }}
            ...
            <Grid container spacing={2}>
              ...
              {/* {selectedEntry.cropName && (
                <Grid item xs={12} sm={6} md={4}>
                  ...
                  <Typography variant="body1" sx={{ fontWeight: 500 }}>{selectedEntry.cropName}</Typography>
                </Grid>
              )} */}
          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <FormControl fullWidth required>
                ...
                <Select value={editFormData.schemeId}
                  onChange={(e) => { setEditFormData({ ...editFormData, schemeId: e.target.value, purposeId: "" }); setEditSelectedScheme(e.target.value); }}
                  label="Scheme">
                  {schemes.map((scheme) => <MenuItem key={scheme.id} value={scheme.id}>{scheme.schemeName}</MenuItem>)}
                </Select>
              </FormControl>
            </Grid>
            <Grid item xs={12}>
              <FormControl fullWidth required>
                ...
                <Select value={editFormData.purposeId}
                  onChange={(e) => setEditFormData({ ...editFormData, purposeId: e.target.value })}
                  label="Purpose" disabled={!editFormData.schemeId}>
                  {purposes.length === 0
                    ? <MenuItem disabled>Select scheme first</MenuItem>
                    : purposes.map((p) => <MenuItem key={p.id} value={p.id}>{p.purposeName}</MenuItem>)}
                </Select>
              </FormControl>
            </Grid>
            <Grid item xs={12}>
              <FormControl fullWidth required>
                ...
                <Select value={editFormData.zoneId}
                  onChange={(e) => setEditFormData({ ...editFormData, zoneId: e.target.value })} label="Zone">
                  {zones.map((zone) => <MenuItem key={zone.zoneId} value={zone.zoneId}>{zone.zoneName || zone.name || `Zone ${zone.zoneId}`}</MenuItem>)}
                </Select>
              </FormControl>
            </Grid>
            <Grid item xs={12}>
              <TextField fullWidth label="Cluster No" value={editFormData.clusterId}
                onChange={(e) => setEditFormData({ ...editFormData, clusterId: validateClusterId(e.target.value,  editFormData.clusterId) })} required />
            </Grid>
            <Grid item xs={12}>
              <FormControl fullWidth>
                <InputLabel>Season</InputLabel>
                <Select value={editFormData.seasonId}
                  onChange={(e) => setEditFormData({ ...editFormData, seasonId: e.target.value })} label="Season">
            ...
            <Grid item xs={12}>
              <FormControl fullWidth>
                ...
                <Select value={editFormData.landType}
                  onChange={(e) => setEditFormData({ ...editFormData, landType: e.target.value })} label="Land Type">
            ...
            <Grid item xs={12} sm={6}>
              <TextField fullWidth label="Distance (km)" value={editFormData.distance}
                onChange={(e) => { const v = validateDistance(e.target.value, editFormData.distance); if (v !== undefined) setEditFormData({ ...editFormData, distance: v }); }} />
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField fullWidth label="Hours" value={editFormData.hours}
                onChange={(e) => { const v = validateHours(e.target.value, editFormData.hours); if (v !== undefined) setEditFormData({ ...editFormData, hours: v }); }} />
            </Grid>
            {/* <Grid item xs={12}>
              <TextField fullWidth label="Crop Name" value={editFormData.cropName}
                onChange={(e) => setEditFormData({ ...editFormData, cropName: validateCropName(e.target.value) })} />
            </Grid> */}
            <Grid item xs={12}>
              <TextField fullWidth label="Location" placeholder="e.g., 10.8505,76.2711" value={editFormData.geoLocation}
                onChange={(e) => setEditFormData({ ...editFormData, geoLocation: validateGeoLocation(e.target.value) })} />
            </Grid>
            <Grid item xs={12}>
              <TextField fullWidth label="Remarks" multiline rows={3} value={editFormData.remark}
      ...
      >
        ...
        >
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            ...
            <IconButton onClick={() => setManualEntryOpen(false)} size="small"><CloseIcon /></IconButton>
          </Box>
          <Divider sx={{ mb: 2 }} />
          <Grid container spacing={2}>
            <Grid item xs={12}>
            ...
            <Grid item xs={12}>
              <FormControl fullWidth required>
                <InputLabel>Purpose</InputLabel>
                <Select value={manualFormData.purposeId}
                  onChange={(e) => setManualFormData({ ...manualFormData, purposeId: e.target.value })}
                  label="Purpose" disabled={!selectedScheme}>
                  {purposes.map((p) => <MenuItem key={p.id} value={p.id}>{p.purposeName}</MenuItem>)}
                </Select>
              </FormControl>
            </Grid>
            <Grid item xs={12}>
              <FormControl fullWidth required>
                <InputLabel>Zone</InputLabel>
                <Select value={manualFormData.zoneId}
                  onChange={(e) => setManualFormData({ ...manualFormData, zoneId: e.target.value })} label="Zone">
                  {zones.length === 0
                    ? <MenuItem disabled>Loading zones...</MenuItem>
                    : zones.map((zone) => <MenuItem key={zone.id} value={zone.zoneId}>{zone.zoneName || zone.name || `Zone ${zone.zoneId}`}</MenuItem>)}
                </Select>
              </FormControl>
            </Grid>
            <Grid item xs={12}>
              <TextField fullWidth label="Cluster number *" value={manualFormData.clusterId}
                onChange={(e) => setManualFormData({ ...manualFormData, clusterId: validateClusterId(e.target.value, manualFormData.clusterId) })} />
            </Grid>
            <Grid item xs={12}>
              <FormControl fullWidth>
                <InputLabel>Season</InputLabel>
                <Select value={manualFormData.seasonId}
                  onChange={(e) => setManualFormData({ ...manualFormData, seasonId: e.target.value })} label="Season">
                  <MenuItem value={1}>Autumn</MenuItem>
                  <MenuItem value={2}>Winter</MenuItem>
                  <MenuItem value={3}>Summer</MenuItem>
                </Select>
            ...
            <Grid item xs={12}>
              <FormControl fullWidth>
                <InputLabel>Land Type</InputLabel>
                <Select value={manualFormData.landType}
                  onChange={(e) => setManualFormData({ ...manualFormData, landType: e.target.value })} label="Land Type">
                  <MenuItem value="WET">WET</MenuItem>
                  <MenuItem value="DRY">DRY</MenuItem>
                </Select>
              </FormControl>
            ...
            <Grid item xs={12} sm={6}>
              <TextField fullWidth label="Distance (km) *" type="text" value={manualFormData.distance}
                onChange={(e) => { const v = validateDistance(e.target.value, manualFormData.distance); if (v !== undefined) setManualFormData({ ...manualFormData, distance: v }); }} />
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField fullWidth label="Hours (hr) *" type="text" value={manualFormData.hours}
                onChange={(e) => { const v = validateHours(e.target.value, manualFormData.hours); if (v !== undefined) setManualFormData({ ...manualFormData, hours: v }); }} />
            </Grid>
            {/* <Grid item xs={12}>
              <TextField fullWidth label="Crop Name" value={manualFormData.cropName}
                onChange={(e) => setManualFormData({ ...manualFormData, cropName: validateCropName(e.target.value) })} />
            </Grid> */}
            <Grid item xs={12}>
              <TextField fullWidth label="Location" placeholder="e.g., 10.8505,76.2711" value={manualFormData.geoLocation}
                onChange={(e) => setManualFormData({ ...manualFormData, geoLocation: validateGeoLocation(e.target.value) })} />
            </Grid>
            <Grid item xs={12}>
              <TextField fullWidth label="Remarks" multiline rows={3} value={manualFormData.remark}
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1, mt: 3 }}>
            ...
            <Button variant="contained" onClick={handleSaveManualEntry} disabled={submitting} startIcon={<SaveIcon />}
              sx={{ backgroundColor: '#27ae60', '&:hover': { backgroundColor: '#229954' } }}>
...
>
  ...
  <DialogContent sx={{ pt: 2.5 }}>
    ...
    {/* <Box sx={{
      ...
      <Typography variant="body2" color="text.secondary">
        Zone ID: <strong style={{ color: theme.palette.text.primary }}>{pendingZoneId}</strong>
      </Typography>
    </Box> */}
  ...
  <DialogActions sx={{ px: 3, pb: 2.5, pt: 1, gap: 1 }}>
    ...
    <Button
      ...
      sx={{ minWidth: 110, backgroundColor: '#27ae60', '&:hover': { backgroundColor: '#229954' } }}
    >
      {submittingMonth ? "Submitting..." : "Submit"}
    </Button>
  </DialogActions>
</Dialog>
...
>
  ...
  <DialogContent sx={{ pt: 2.5 }}>
    <Box sx={{
      ...
      border: `1px solid ${
        resultDialog.type === 'success' ? '#c8e6c9' :
