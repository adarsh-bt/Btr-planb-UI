...
const UserTourDiaryDetail = () => {
  ...
  const fetchUserTourEntries = async () => {
    ...
    try {
      ...
      if (data && data.message) {
        ...
        // Enhance data with purpose names if needed
        const enhancedData = data.map(entry => {
          // If purposeName is not provided by API, try to find it from allPurposes
        ...
        // Extract user details from the first entry if available
        if (data.length > 0 && !userDetails) {
  ...
  // Check if a half is already approved
  const isHalfApproved = (half) => {
    if (!submissionDetails) return false;
    
    // This is a simplified check - you might need to check the actual status
    // of the submission from another API or from the submission details
    return false; // Placeholder - implement based on your actual data structure
  };
        ...
        <MainCard>
          <Box sx={{ maxWidth: '1000px', margin: '0 auto' }}>
            ...
            >
              ...
              {/* Right section with approval button and navigation */}
              <Box sx={{ 
                display: 'flex', 
                alignItems: 'center', 
                gap: 2,
