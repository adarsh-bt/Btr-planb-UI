...
const roleManageService = {
  ...
  async getPermissions(schemeId) {
    ...
    console.log('token', token);
    try {
      const response = await axios.get(`${BASE_URL}/api/fetch/permissions/${schemeId}`, {
        headers: {
          Authorization: `Bearer ${token}` // Add token in Authorization header
        }
      });
  ...
  async saveOrUpdateRole(userData) {
    const token = localStorage.getItem('token');
    try {
      const response = await axios.post(`${BASE_URL}/api/fetch/save-role-permissions`, userData, {
