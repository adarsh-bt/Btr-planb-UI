...
const TEXT_COLS = [{ label: 'Taluk Name', width: '35%', get: (r) => r.name }];
const TOTAL_LABELS = ['District Total / Summary', 'Total Excluded Summary', 'Total Available Summary'];
...
function TalukWorkAllocationReport() {
  ...
  // districtId arrives in navigation state; on a direct/refresh load it is
  // resolved once from the state list and cached in a ref.
  const stateDistrictId = location.state?.districtId ?? location.state?.districtData?.id ?? null;
  const resolvedDistrictId = useRef(stateDistrictId);
  ...
  const resolveDistrictId = useCallback(async () => {
    if (resolvedDistrictId.current) return resolvedDistrictId.current;
    ...
    resolvedDistrictId.current =
      list.find((d) => String(d.name).toLowerCase() === districtName.toLowerCase())?.id ?? null;
    return resolvedDistrictId.current;
  }, [districtName]);

  const fetchTalukWorkAllocation = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const districtId = await resolveDistrictId();
      if (!districtId) throw new Error(`Could not resolve District ID for "${districtName}".`);
      ...
      console.error('Error fetching Taluk work allocation report:', err);
      setError(err.response?.data?.message || err.message || 'Error loading taluk work allocation report data.');
    } finally {
      setLoading(false);
    }
  }, [districtName, resolveDistrictId]);

  // Taluk level users only ever see their own zone page.
  useEffect(() => {
    ...
    fetchTalukWorkAllocation();
  }, [jurisdiction.level, jurisdiction.district, jurisdiction.taluk, navigate, fetchTalukWorkAllocation]);
  ...
  const handleDrillDownToZone = (row) => {
    navigate(
      `/report/kerala_work_allocation_report/zone/${encodeURIComponent(districtName)}/${encodeURIComponent(row.name)}`,
      {
        state: {
  ...
  const reportLevelName = `${districtName} District Report (${jurisdiction.role})`;
  ...
  return (
    <Grid container spacing={3}>
      ...
      <Grid item xs={12}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1, flexWrap: 'wrap', gap: 2 }}>
          <Stack direction="row" spacing={2} alignItems="center">
            {jurisdiction.level !== 'taluk' && (
              <Button
                ...
                onClick={() => navigate('/report/kerala_work_allocation_report')}
          ...
          <Button
            ...
            onClick={fetchTalukWorkAllocation}
            disabled={loading}
            sx={{ borderRadius: 2 }}
          >
            Refresh
      ...
      {/* Main Table Card */}
      <Grid item xs={12}>
        <MainCard title={`Work Allocation Abstract - Taluks of ${districtName} (${rows.length} Taluks)`}>
          <Box sx={{ width: '100%' }}>
                  ...
                  {loading ? (
                    ...
                    paginatedData.map((row, index) => (
                      <TableRow
                        ...
                        onClick={() => handleDrillDownToZone(row)}
                      >
