...
const TalukOfficeManagementTab = () => {
  ...
  const fetchTalukOffices = useCallback(async () => {
    ...
    try {
      ...
      if (response.status === 403) throw new Error("Access denied");
      if (!response.ok) throw new Error(`Failed to fetch taluk offices: ${response.status}`);
  ...
  const saveTalukOffice = async (payload) => {
    const token = localStorage.getItem("token");
    if (!token) throw new Error("No authentication token found");
  ...
  const handleSaveNewOffice = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        distId: Number(newOfficeData.distId),
        desTalukId: Number(newOfficeData.desTalukId),
        userId: userId,
...
const RevenueTalukManagementTab = () => {
  ...
  const fetchRevenueTaluks = useCallback(async () => {
    ...
    try {
      ...
      if (response.status === 403) throw new Error("Access denied");
      if (!response.ok) throw new Error(`Failed to fetch revenue taluks: ${response.status}`);
  ...
  const saveRevenueTaluk = async (payload) => {
    const token = localStorage.getItem("token");
    if (!token) throw new Error("No authentication token found");
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      await saveRevenueTaluk(payload);
  ...
  if (error) {
    return (
      <Alert severity="error" sx={{ m: 3 }} action={<Button color="inherit" size="small" onClick={fetchRevenueTaluks}>Retry</Button>}>
        <Typography variant="subtitle1" fontWeight="bold">Access Error</Typography>
        <Typography>{error}</Typography>
      </Alert>
    );
    ...
    <>
      <Paper elevation={2} sx={{ p: 2, mb: 3, display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
        <TextField
          ...
          sx={{ minWidth: 250 }}
        />
        <Button variant="outlined" startIcon={<Refresh />} onClick={fetchRevenueTaluks}>Refresh</Button>
        <Button variant="contained" startIcon={<AddIcon />} onClick={handleAddClick} sx={{ ml: 'auto' }}>
      ...
      <Paper elevation={3}>
        <TableContainer sx={{ maxHeight: 'calc(100vh - 350px)' }}>
          <Table stickyHeader>
            ...
            <TableBody>
              {filteredTaluks.length === 0 ? (
                <TableRow><TableCell colSpan={10} align="center">No revenue taluks found</TableCell></TableRow>
              ) : (
                filteredTaluks.map((taluk) => (
                  <TableRow key={taluk.revTalukId} sx={{ backgroundColor: !taluk.isActive ? '#fafafa' : 'inherit' }}>
                    ...
                    <TableCell sx={{ fontFamily: 'Noto Sans Malayalam, "Malayalam MN", sans-serif' }}>{taluk.revTalukNameMal}</TableCell>
                    <TableCell>{getDistrictName(taluk.distId)}</TableCell>
      ...
      <Dialog open={addModalOpen} onClose={() => setAddModalOpen(false)} maxWidth="md" fullWidth>
        <DialogTitle sx={{ bgcolor: '#05307a', color: 'white', pb: 2 }}>
          <Typography variant="h6">Add New Revenue Taluk</Typography>
        </DialogTitle>
        <DialogContent sx={{ pt: 3, pb: 2 }}>
          <Grid container spacing={2.5} sx={{ mt: 1 }}>
            <Grid item xs={12} sm={6}>
              <FormControl fullWidth required error={!!addErrors.distId}>
                <InputLabel>Select District</InputLabel>
                <Select value={newTalukData.distId} label="Select District" onChange={(e) => handleAddChange('distId', e.target.value)}>
                  {districts.map((district) => (
                    <MenuItem key={district.dist_id} value={district.dist_id}>
                      {district.dist_name_en} / {district.dist_name_mal}
                    </MenuItem>
      ...
      <Dialog open={editModalOpen} onClose={() => setEditModalOpen(false)} maxWidth="md" fullWidth>
        <DialogTitle sx={{ bgcolor: '#05307a', color: 'white', pb: 2 }}>
          <Typography variant="h6">Edit Revenue Taluk</Typography>
          <Typography variant="body2">ID: {selectedTaluk?.revTalukId} | Current: {selectedTaluk?.revTalukNameEn}</Typography>
        </DialogTitle>
        <DialogContent sx={{ pt: 3, pb: 2 }}>
          <Grid container spacing={2.5} sx={{ mt: 1 }}>
            <Grid item xs={12} sm={6}>
