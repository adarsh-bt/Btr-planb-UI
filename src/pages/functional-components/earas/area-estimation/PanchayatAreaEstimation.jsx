...
const ZONES_DATA = [
  { id: 201, name: 'Zone KLM-01', estimatedArea: 320, recordCount: 1 },
  { id: 202, name: 'Zone KLM-02', estimatedArea: 380, recordCount: 2 },
  { id: 203, name: 'Zone KLM-03', estimatedArea: 250, recordCount: 1 }
];

const PanchayatAreaEstimation = () => {
  const { districtId, blockId, panchayatId } = useParams();
  ...
  // Dynamically calculate crop areas for each zone
  const rowData = ZONES_DATA.map(row => {
    const cropAreas = {};
  ...
  const handleExportExcel = () => {
    ...
    const filename = `Panchayat_${panchayatId}_Area_Estimation_${year}_${season}_${dateStr}_${timeStr}.xls`;
    ...
    if (isCropType) {
      ...
      dataRows = rowData.map(z => {
        const area = getArea(z, selectedCrop, landType);
        const prevArea = getPrevYearArea(z, selectedCrop, landType);
        ...
        return [z.name, prevIrr, irr, prevUnirr, unirr, prevArea, area, `${yoy}%`];
      });
    } else {
      headers = [
        'Zone Name',
      ...
      dataRows = rowData.map(z => {
        const totalArea = categories.reduce((sum, cat) => sum + getArea(z, cat, landType), 0);
        const prevTotalArea = categories.reduce((sum, cat) => sum + getPrevYearArea(z, cat, landType), 0);
        const yoy = prevTotalArea > 0 ? (((totalArea - prevTotalArea) / prevTotalArea) * 100).toFixed(2) : '0.00';
        return [
          z.name,
          ...categories.flatMap(cat => {
            const area = getArea(z, cat, landType);
            const prevArea = getPrevYearArea(z, cat, landType);
    ...
    const metadataRows = [
      ['Report Name', 'Panchayat Level Area Estimation'],
    ...
    exportToExcelMultiSheet(filename, 'Panchayat Estimation', headers, dataRows, metadataRows);
  };

  const filteredRowData = rowData.filter(z =>
    ...
    <Grid container spacing={3}>
      ...
      <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Typography variant="h3" fontWeight={700} sx={{ color: '#04255e' }}>
          Detailed Estimation - Panchayat Level (ID: {panchayatId}) ({estType} {estType === 'Seasonal Crops' ? `- ${season}` : ''})
        </Typography>
          ...
          {isCropType ? (
              ...
              <Grid item xs={12} sm={4}>
                <Card sx={{ bgcolor: '#04255e', color: 'white', borderRadius: 3 }}>
                  <CardContent>
                    ...
                    <Typography variant="caption" sx={{ opacity: 0.6 }}>({landType})</Typography>
                  </CardContent>
                </Card>
              </Grid>
              <Grid item xs={12} sm={4}>
            ...
            <>
              <Grid item xs={12} sm={4}>
                <Card sx={{ bgcolor: '#04255e', color: 'white', borderRadius: 3 }}>
                  <CardContent>
                    <Typography variant="h6" sx={{ opacity: 0.8 }}>Total Panchayat Estimated Area</Typography>
      ...
      <Grid item xs={12}>
        <MainCard
          title={`${estType} Contribution details`}
          secondary={
            ...
            <Table>
              <TableHead sx={{ bgcolor: '#04255e' }}>
                {isCropType ? (
                  <>
                    <TableRow>
                      <TableCell rowSpan={2} sx={{ color: 'white', fontWeight: 600 }}>Zone Name</TableCell>
              ...
              <TableBody>
                ...
                {filteredRowData.length > 0 && (
                  <TableRow sx={{ bgcolor: 'rgba(0,0,0,0.02)', fontWeight: 'bold' }}>
                    <TableCell sx={{ fontWeight: 'bold' }}>Total Panchayat Area</TableCell>
                    {isCropType ? (
                      <>
