...
/**
 * Seasonal crop results at Panchayat level.
 *
...
const SeasonalBlockAreaEstimation = () => {
  ...
  const parentName = results.breakdown?.parentGeoName || 'Block';

  const handleDrillDown = (row) => {
    navigate(`/schemes/earas/area-estimation/seasonal/${districtId}/${blockId}/${row.geoId}`, { state: { estType, season } });
  };
  ...
  return (
    <Grid container spacing={3} className="printable-area">
      ...
      <Grid item xs={12}>
        <MainCard
          title="Panchayat-wise Seasonal Crop Area"
          secondary={
