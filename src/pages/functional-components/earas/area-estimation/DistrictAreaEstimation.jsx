...
const DistrictAreaEstimation = () => {
  ...
  const handleViewDetails = (blockId) => {
    navigate(`/schemes/earas/area-estimation/${districtId}/${blockId}`, { state: { estType, season } });
  };
  ...
  const handleExportExcel = () => {
    ...
    const filename = `District_${districtId}_Area_Estimation_${year}_${season}_${dateStr}_${timeStr}.xls`;
    ...
    if (isCropType) {
      ...
      dataRows = rowData.map(b => {
        const totalArea = categories.reduce((sum, cat) => sum + getArea(b, cat, landType), 0);
        const prevTotalArea = categories.reduce((sum, cat) => sum + getPrevYearArea(b, cat, landType), 0);
        const yoy = prevTotalArea > 0 ? (((totalArea - prevTotalArea) / prevTotalArea) * 100).toFixed(2) : '0.00';
        return [
          b.name,
          ...categories.flatMap(cat => {
            const area = getArea(b, cat, landType);
            const prevArea = getPrevYearArea(b, cat, landType);
    ...
    const metadataRows = [
      ['Report Name', 'District Level Area Estimation'],
    ...
    exportToExcelMultiSheet(filename, 'District Estimation', headers, dataRows, metadataRows);
  };
    ...
    <Grid container spacing={3}>
      ...
      <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Typography variant="h3" fontWeight={700} sx={{ color: '#04255e' }}>
          District Level Area Estimation (District ID: {districtId}) ({estType} {estType === 'Seasonal Crops' ? `- ${season}` : ''})
        </Typography>
          ...
          {isCropType ? (
            ...
            <>
              <Grid item xs={12} sm={6}>
                <Card sx={{ bgcolor: '#04255e', color: 'white', borderRadius: 3 }}>
                  <CardContent>
                    <Typography variant="h6" sx={{ opacity: 0.8 }}>Total District Estimated Area</Typography>
              ...
              <Grid item xs={12} sm={6}>
                <Card sx={{ bgcolor: '#37474f', color: 'white', borderRadius: 3 }}>
                  <CardContent>
                    <Typography variant="h6" sx={{ opacity: 0.8 }}>Total Blocks</Typography>
                    <Typography variant="h2" fontWeight={700} sx={{ mt: 1 }}>{BLOCKS_DATA.length}</Typography>
                    <Typography variant="caption" sx={{ opacity: 0.6 }}>Reporting blocks</Typography>
                  </CardContent>
                </Card>
              </Grid>
            </>
