...
/**
 * Seasonal crop results at District level.
 *
...
const SeasonalStateAreaEstimation = () => {
  ...
  const parentName = results.breakdown?.parentGeoName || 'Kerala State';

  const handleDrillDown = (row) => {
    navigate(`/schemes/earas/area-estimation/seasonal/${row.geoId}`, { state: { estType, season } });
  };
  ...
  return (
    <Grid container spacing={3} className="printable-area">
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} sm={4}>
            <Card sx={{ bgcolor: '#04255e', color: 'white', borderRadius: 3 }}>
              <CardContent>
                <Typography variant="h6" sx={{ opacity: 0.8 }}>State Estimated Area</Typography>
          ...
          <Grid item xs={12} sm={4}>
            <Card sx={{ bgcolor: '#1b5e20', color: 'white', borderRadius: 3 }}>
              <CardContent>
                <Typography variant="h6" sx={{ opacity: 0.8 }}>Total Across Districts</Typography>
                <Typography variant="h2" fontWeight={700} sx={{ mt: 1 }}>
                  {formatHectares(results.levelTotalAreaHa)} Ha
                </Typography>
                <Typography variant="caption" sx={{ opacity: 0.6 }}>
                  {results.totalRows} districts
                </Typography>
              </CardContent>
            </Card>
          </Grid>
      ...
      <Grid item xs={12}>
        <MainCard
          title="District-wise Seasonal Crop Area"
          secondary={
        ...
        >
          ...
          showIrrigationSplit={true}
            ...
            rows={filteredRows}
            unitLabel="District"
            totalsLabel="Total Across Districts"
