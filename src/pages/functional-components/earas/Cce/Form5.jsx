  ...
  useEffect(() => {
    const fetchTableData = async () => {
      try {
        ...
        const response = await fetch(`${mainapi.FORM_API}/earas-form1-entry/cce-crop-details/fetch-cce-crops?zoneId=${zoneid}`, {
          method: 'GET',
          headers: {
        ...
        const dataArray = Array.isArray(data) ? data : (data.payload || data.data || []);

        // Map the API fields to our table columns
        const mappedData = dataArray.map((item, index) => ({
          id: item.cceAvailablePlotId || item.id || index,
