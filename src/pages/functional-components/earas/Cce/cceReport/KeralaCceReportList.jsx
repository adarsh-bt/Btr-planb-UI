...
function KeralaCceReportList() {
  ...
  const cropsList = ['ALL', 'Paddy', 'Coconut', 'Banana', 'Tapioca', 'Pepper'];

  // Mock data for CCE Progress (Aggregated at District level)
  const mockApiData = useMemo(() => {
    ...
    return districts.map((district, index) => {
      // Mock some variation based on district index and selected crop
      const multiplier = selectedCrop === 'ALL' ? 3 : 1;
      const baseVal = (14 - index) * 10 * multiplier;
  ...
  const handleClearFilters = () => {
    setSelectedCrop('ALL');
    setAgriculturalYear('2024-25');
    setPage(0);
    setSearchTerm('');
  };

  const handleViewDetails = (districtName, districtId) => {
    navigate(`/kerala_cce_report/taluk_cce_report/${districtName}`, {
      state: {
        districtId,
        districtName,
        selectedCrop,
    ...
    <Grid container spacing={3}>
          ...
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            <Box>
              <Typography variant="h4" sx={{ fontWeight: 'bold', color: '#1a237e' }}>
                State-wise CCE Progress Report
              </Typography>
              <Typography variant="body2" color="text.secondary">
                {`Agricultural Year: ${agriculturalYear} • Crop: ${selectedCrop}`}
              </Typography>
            </Box>
          </Stack>
          {(selectedCrop !== 'ALL' || agriculturalYear !== '2024-25') && (
            <Button
              variant="outlined"
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems="center" flexWrap="wrap">
            <FormControl size="small" sx={{ minWidth: 200 }}>
              ...
              <Select
                ...
                onChange={(e) => { setAgriculturalYear(e.target.value); setPage(0); }}
              >
            ...
            <FormControl size="small" sx={{ minWidth: 200 }}>
              ...
              <Select
                value={selectedCrop}
                label="Select Crop"
                onChange={(e) => { setSelectedCrop(e.target.value); setPage(0); }}
              >
                {cropsList.map(crop => <MenuItem key={crop} value={crop}>{crop}</MenuItem>)}
              </Select>
            </FormControl>
          </Stack>
        </Paper>
      ...
      <Grid item xs={12}>
        <Box sx={{ position: 'relative', borderRadius: 3 }}>
          <Chip
            label="District Progress Summary"
            color="primary"
            size="small"
            sx={{
              position: 'absolute',
          ...
          <MainCard
            title="District-wise CCE Progress"
            secondary={
              <TextField
                ...
                InputProps={{
                  ...
                  endAdornment: searchTerm && (
                    <InputAdornment position="end">
                      <IconButton size="small" onClick={handleClearFilters} edge="end">
              ...
              <Table>
                <TableHead>
                  <TableRow sx={{ bgcolor: '#04255e' }}>
                    {['District', 'Total', 'Completed', 'Ongoing', 'Not Available', 'Not Started', 'Under Review', 'Actions'].map((label, idx) => (
                      ...
                      >
                        ...
                        <TableCell align="center">
                          <Tooltip title="View Details">
                            <IconButton
                              size="small"
                              onClick={() => handleViewDetails(row.district, row.id)}
...
export default KeralaCceReportList;
