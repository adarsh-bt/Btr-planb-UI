...
function TalukCceReport() {
  const theme = useTheme();
  const navigate = useNavigate();
  const location = useLocation();
  const { districtName: urlDistrictName } = useParams();
  ...
  // Mock data for CCE Progress (Aggregated at Taluk level)
  const mockApiData = useMemo(() => {
    // Generate some mock taluks based on district name length to make it deterministic
    const talukCount = (districtName.length % 3) + 3; // 3 to 5 taluks
    const taluks = Array.from({ length: talukCount }, (_, i) => `${districtName} Taluk ${i + 1}`);
    
    return taluks.map((taluk, index) => {
      // Mock some variation
      const multiplier = selectedCrop === 'ALL' ? 3 : 1;
      const baseVal = (talukCount - index + 2) * 10 * multiplier;
  ...
  }, [districtName, selectedCrop, agriculturalYear]);
  ...
  const handleViewDetails = (talukName, talukId) => {
    navigate(`/kerala_cce_report/taluk_cce_report/zone_cce_report/${districtName}/${talukName}`, {
      state: { 
        districtName,
        talukName,
        talukId,
  ...
  return (
    <Grid container spacing={3}>
          ...
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            <Box>
              <Typography variant="h4" sx={{ fontWeight: 'bold', color: '#1a237e' }}>
                Taluk-wise CCE Progress: {districtName}
              </Typography>
      ...
      <Grid item xs={12}>
        ...
        >
          <Chip
            label={`${districtName} District Summary`}
            color="primary"
            size="small"
            sx={{
              position: 'absolute',
              ...
              <Table>
                <TableHead>
                  <TableRow sx={{ bgcolor: '#04255e' }}>
                    {['Taluk', 'Total', 'Completed', 'Ongoing', 'Not Available', 'Not Started', 'Under Review', 'Actions'].map((label, idx) => (
                ...
                <TableBody>
                  {loading ? (
                      ...
                      >
                        ...
                        <TableCell align="center">
                          <Tooltip title="View Details">
                            <IconButton 
                              size="small"
                              onClick={() => handleViewDetails(row.taluk, row.id)}
                    ...
                    <TableRow>
                      <TableCell colSpan={8} align="center" sx={{ py: 6 }}>
                        <Typography color="text.secondary">No taluks found</Typography>
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
