...
// Fetch saved allocations and read their approval status
  ...
  const handleInputChange = (index, field, value) => {
    ...
    // Validation for Wet_area, Dry_area and Total_area
if (['Wet_area', 'Dry_area', 'Total_area'].includes(field)) {
  ...
  if (
   ...
   // Allow only numbers, max 10 digits before decimal, max 2 digits after decimal
  ...
  <Grid container spacing={2} justifyContent="flex-end">
    <Grid item>
      <Tooltip title={isDisabled ? 'Form parameters are currently locked' : 'Save entries as draft'}>
        <span>
          <ActionButton variant="outlined" color="primary" onClick={handleSaveDraft} disabled={role !== 'Field Data Collector' || isDisabled || isSubmitting} startIcon={<SaveIcon />}>
    ...
    <Grid item>
      <Tooltip title={!canSubmit && !isDisabled ? 'Please complete all metric fields' : ''}>
        <span>
          <ActionButton variant="contained" color="success" onClick={handleSubmitClick} disabled={role !== 'Field Data Collector' || isDisabled || !canSubmit || isSubmitting} startIcon={<SendIcon />}>
